...
// Funci�n para hacer la solicitud Fetch
function hacerFetch(url, datos) {
    ...
    .catch(error => {
        ...
        mostrarMensaje('Ocurri� un error al comunicar con el servidor.');
    });
}

// Funci�n para convertir hora en formato HH:MM a minutos
        ...
        .then(datos => {
            ...
            if (datos.error) {
                ...
                const elementos = [
                    ...
                    `C�digo �nico: ${datos.codigo_unico}`,
                    `Franja Horaria M�nima: ${datos.franja_horaria_min || 'No asignado'}`,
                    `Franja Horaria M�xima: ${datos.franja_horaria_max || 'No asignado'}`
                ];
...
formularioAsignarHorario.addEventListener('submit', function(evento) {
    ...
    if (minutosMin >= minutosMax) {
        mostrarMensaje("El horario m�nimo debe ser menor que el horario m�ximo.");
    } else {
        const horario = {
        ...
        hacerFetch(urlServidor, horario)
            .then(datos => {
                if (datos.error) {
                    ...
                    mostrarMensaje(datos.resultado || "Horario asignado con �xito.");
                }
            });
    }
});
